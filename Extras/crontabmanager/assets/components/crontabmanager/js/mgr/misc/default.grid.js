CronTabManager.grid.Default = function (config) {
    config = config || {}
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper crontabmanager-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('crontabmanager-row-unpublished')
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('crontabmanager-row-inactive')
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('crontabmanager-row-deleted')
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('crontabmanager-row-required')
                }
                return cls.join(' ')
            }
        },
        processors: {
            ...
            message: function (action, btn, e, row) {
                CronTabManager.processors.message(this.element, action, btn, e, row)
            },
            one: function (action, btn, e, row) {
                CronTabManager.processors.one(this.element, action, btn, e, row)
            },
            confirm: function (action, lexicon, params, callback) {
                CronTabManager.processors.run(this.element, action, lexicon, params, callback, 'confirm')
            },
            multiple: function (action, lexicon, params, callback) {
                CronTabManager.processors.run(this.element, action, lexicon, params, callback,'multiple')
            }
        },
    })
    CronTabManager.grid.Default.superclass.constructor.call(this, config)
...
Ext.extend(CronTabManager.grid.Default, MODx.grid.Grid, {
    ...
    getColumns: function () {
        return [{
            ...
            header: _('crontabmanager_actions'),
            dataIndex: 'actions',
    ...
    getSearchField: function (width) {
        return {
            xtype: 'crontabmanager-field-search',
            width: width || 250,
            listeners: {
                search: {
                    fn: function (field) {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = CronTabManager.utils.getMenu(row.data['actions'], this, ids)
...
Ext.reg('crontabmanager-grid-default', CronTabManager.grid.Default)
