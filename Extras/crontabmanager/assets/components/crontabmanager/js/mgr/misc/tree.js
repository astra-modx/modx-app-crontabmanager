CronTabManager.tree.Categories = function (config) {
    config = config || {}
    if (!config.id) {
        config.id = 'crontabmanager-categories-tree'
    }
    ...
    CronTabManager.tree.Categories.superclass.constructor.call(this, config)
}
Ext.extend(CronTabManager.tree.Categories, MODx.tree.Tree, {
    ...
    _handleCheck: function (id, checked) {
        ...
        var value = Ext.util.JSON.decode(val)
        if (val === '{}' && this.config.categories) {
            value = Ext.util.JSON.decode(this.config.categories)
        }
...
Ext.reg('crontabmanager-tree-categories', CronTabManager.tree.Categories)
