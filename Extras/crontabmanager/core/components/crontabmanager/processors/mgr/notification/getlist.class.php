...
class CronTabManagerNotificationGetListProcessor extends modObjectGetListProcessor
{
    ...
    public function prepareQueryBeforeCount(xPDOQuery $c)
    {
        ...
        $c->leftJoin('CronTabManagerRule', 'Rule', 'Rule.id = CronTabManagerNotification.rule_id');
        $c->select('Rule.name as rule_name, Rule.class as rule_class');
    ...
    public function prepareRow(xPDOObject $object)
    {
        ...
        if ($object->get('send') == 0) {
            // Send
            $array['actions'][] = array(
                'cls' => '',
                'icon' => 'icon icon-send-o action-green',
                'title' => $this->modx->lexicon('crontabmanager_notification_action_send'),
                #'multiple' => $this->modx->lexicon('crontabmanager_notifications_send'),
                'action' => 'sendItem',
                'button' => true,
                'menu' => true,
            );
        ...
        $array['actions'][] = array(
            ...
            'multiple' => $this->modx->lexicon('crontabmanager_notifications_remove'),
            'action' => 'removeItem',
            'button' => true,
            'menu' => true,
        );
